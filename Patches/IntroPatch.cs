...
//        }
        ...
        class SetUpRoleTextPatch {
            ...
            static public IEnumerator SetRoleTexts(IntroCutscene __instance) {
                ...
                if (Deputy.knowsSheriff && Deputy.deputy != null && TimespaceManager.timespaceManager != null) {
                    if (infos.Any(info => info.roleId == RoleId.TimespaceManager))
                        __instance.RoleBlurbText.text += Helpers.cs(TimespaceManager.color, string.Format(ModTranslation.getString("deputyIntroLine"), Deputy.deputy?.Data?.PlayerName ?? ""));
                    else if (infos.Any(info => info.roleId == RoleId.Deputy))
                        __instance.RoleBlurbText.text += Helpers.cs(TimespaceManager.color, string.Format(ModTranslation.getString("sheriffIntroLine"), TimespaceManager.timespaceManager?.Data?.PlayerName ?? ""));
                }
