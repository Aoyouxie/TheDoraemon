    ...
    public static class OnGameEndPatch
    {
        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)]ref EndGameResult endGameResult) {
            ...
            foreach(PlayerControl playerControl in PlayerControl.AllPlayerControls) {
                ...
                if (killCount == 0 && !(new List<RoleInfo>() { RoleInfo.timespaceManager, RoleInfo.jackal, RoleInfo.sidekick, RoleInfo.thief, RoleInfo.moriarty, RoleInfo.jekyllAndHyde, RoleInfo.doomsayer }.Contains(RoleInfo.getRoleInfoForPlayer(playerControl, false).FirstOrDefault()) || playerControl.Data.Role.IsImpostor)) {
                    killCount = null;
                    }
            ...
            {
                EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
                CachedPlayerData wpd = new (Kataomoi.kataomoi.Data);
                EndGameResult.CachedWinners.Add(wpd);
            ...
            if (Pursuer.pursuer != null && !Pursuer.pursuer.Data.IsDead && !Pursuer.notAckedExiled && !saboWin && !arsonistWin)
            {
            ...
            foreach (var playerInfo in GameData.Instance.AllPlayers.GetFastEnumerator())
            {
                if (!playerInfo.Disconnected)
                {
                    if (!playerInfo.IsDead)
                    {
                        ...
                        if (TimespaceManager.timespaceManager != null && TimespaceManager.timespaceManager.PlayerId == playerInfo.PlayerId && !Madmate.madmate.Contains(TimespaceManager.timespaceManager)) {
                            numSheriffAlive++;
                        }
