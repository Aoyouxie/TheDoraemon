...
namespace TheDoraemon.Patches {
    ...
    class GameOptionsDataGetAdjustedNumImpostorsPatch {
        public static void Postfix(ref int __result) {
            ...
            else if (GameOptionsManager.Instance.CurrentGameOptions.GameMode == GameModes.Normal) {  // Ignore Vanilla impostor limits in TDR Games.
    ...
    class RoleManagerSelectRolesPatch {
        ...
        private static void selectFactionForFactionIndependentRoles(RoleAssignmentData data) {
            ...
            // Assign TimespaceManager
            if ((CustomOptionHolder.deputySpawnRate.getSelection() > 0 &&
                CustomOptionHolder. timespaceManagerSpawnRate.getSelection() == 10) ||
                CustomOptionHolder.deputySpawnRate.getSelection() == 0) 
                    data.crewSettings.Add((byte)RoleId.TimespaceManager, CustomOptionHolder.timespaceManagerSpawnRate.getSelection());
        ...
        private static void assignDependentRoles(RoleAssignmentData data) {
            ...
            bool sheriffFlag = CustomOptionHolder.deputySpawnRate.getSelection() > 0 
                && CustomOptionHolder.timespaceManagerSpawnRate.getSelection() > 0;
            ...
            while (crew > 0 && (!isSheriff || (!isEvilGuesser && !isGuesser) || (!isEvilWatcher && !isWatcher))) {
                if (!isSheriff && rnd.Next(crewValues) < CustomOptionHolder.timespaceManagerSpawnRate.getSelection()) isSheriff = true;
            ...
            if (isSheriff && TimespaceManager.timespaceManager == null && data.crewmates.Count > 0 && data.maxCrewmateRoles > 0 && sheriffFlag) { // Set TimespaceManager cause he won the lottery
                byte timespaceManager = setRoleToRandomPlayer((byte)RoleId.TimespaceManager, data.crewmates);
                data.crewmates.ToList().RemoveAll(x => x.PlayerId == timespaceManager);
            ...
            if (TimespaceManager.timespaceManager != null) { // Deputy
            ...
            if (!data.crewSettings.ContainsKey((byte)RoleId.TimespaceManager)) data.crewSettings.Add((byte)RoleId.TimespaceManager, 0);
        ...
        public static void assignModifiers() {
            ...
            crewPlayerMadmate.RemoveAll(x => x.Data.Role.IsImpostor || Helpers.isNeutral(x)|| x == FortuneTeller.fortuneTeller || x == Sprinter.sprinter || x == Veteran.veteran
            ...
            // Always remember to remove the Mad TimespaceManager if Deputy is assigned
            if (Deputy.deputy != null && TimespaceManager.timespaceManager != null) crewPlayerMadmate.RemoveAll(x => x == TimespaceManager.timespaceManager);
            ...
            if (rnd.Next(1, 101) <= CustomOptionHolder.modifierLover.getSelection() * 10) { // Assign lover
                ...
                crewPlayer.RemoveAll(x => x.Data.Role.IsImpostor|| x == FortuneTeller.fortuneTeller || x == Akujo.akujo || x == Cupid.cupid || x == Fox.fox
                || x == Kataomoi.kataomoi);
