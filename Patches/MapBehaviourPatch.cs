...
namespace TheDoraemon.Patches {
	...
	class MapBehaviourPatch {
        ...
        public static Sprite Vent = Helpers.loadSpriteFromResources("TheDoraemon.Resources.Vent.png", 150f);
        ...
        private static bool evilTrackerShowTask(MapTaskOverlay __instance)
        {
            ...
            for (int i = 0; i < realTasks[EvilTracker.target.PlayerId].Count; i++)
            {
                ...
                {
                    TheDoraemonPlugin.Logger.LogError(ex.Message);
                }
            }
            return false;
        }
		...
		static void Postfix(MapBehaviour __instance) {
            ...
            if (EvilHacker.canSeeDoorStatus && ((EvilHacker.evilHacker != null && PlayerControl.LocalPlayer.PlayerId == EvilHacker.evilHacker.PlayerId) || EvilHacker.isInherited()))
            {
                ...
                {
                    doorClosedSprite = Helpers.loadSpriteFromResources("TheDoraemon.Resources.Cross.png", 500f);
                }
                doorMarks ??= new();
            ...
            foreach (var vent in MapUtilities.CachedShipStatus.AllVents)
            {
                ...
                if (!TheDoraemonPlugin.ShowVentsOnMap.Value)
                {
                    if (mapIcons.Count > 0)
                    {
