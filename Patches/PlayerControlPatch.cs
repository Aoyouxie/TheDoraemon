...
using static TheDoraemon.Helpers;
...
namespace TheDoraemon.Patches {
    ...
    public static class PlayerControlFixedUpdatePatch {
        ...
        static void sheriffSetTarget() {
            if (TimespaceManager.timespaceManager == null || TimespaceManager.timespaceManager != PlayerControl.LocalPlayer) return;
            TimespaceManager.currentTarget = setTarget();
            setPlayerOutline(TimespaceManager.currentTarget, TimespaceManager.color);
        }
        ...
        {
            // If LocalPlayer is Deputy, the TimespaceManager is disconnected and Deputy promotion is enabled, then trigger promotion
            if (Deputy.deputy == null || Deputy.deputy != PlayerControl.LocalPlayer) return;
            if (Deputy.promotesToSheriff == 0 || Deputy.deputy.Data.IsDead == true || Deputy.promotesToSheriff == 2 && !isMeeting) return;
            if (TimespaceManager.timespaceManager == null || TimespaceManager.timespaceManager?.Data?.Disconnected == true || TimespaceManager.timespaceManager.Data.IsDead)
            {
        ...
        public static void playerSizeUpdate(PlayerControl p) {
            ...
            if ((Morphling.morphling != null && p == Morphling.morphling && Morphling.morphTimer > 0f) ||
                (MimicA.mimicA != null && p == MimicA.mimicA && MimicA.isMorph && MimicK.mimicK != null)) {
            }
        }
        ...
        public static void trapperUpdate()
        {
            ...
            {
                TheDoraemonPlugin.Logger.LogWarning(e.Message);
            }
        }
        ...
        public static void updatePlayerInfo() {
            ...
            foreach (PlayerControl p in PlayerControl.AllPlayerControls) {
                ...
                if ((PlayerControl.LocalPlayer|| (Akujo.knowsRoles && PlayerControl.LocalPlayer == Akujo.akujo && (p == Akujo.honmei || Akujo.keeps.Any(x => x.PlayerId == p.PlayerId))) || p == PlayerControl.LocalPlayer || (PlayerControl.LocalPlayer.Data.IsDead
                   ) || FreePlayGM.isFreePlayGM) ){
                    ...
                    else if (TORMapOptions.ghostsSeeRoles || (PlayerControl.LocalPlayer)) {
                        playerInfoText = $"{roleText}";
                        meetingInfoText = playerInfoText;
                    }
        ...
        static void assassinSetTarget()
        {
            ...
            Assassin.currentTarget = setTarget(untargetablePlayers: untargetables);
        ...
        public static void Postfix(PlayerControl __instance) {
            ...
            if (PlayerControl.LocalPlayer == __instance) {
                ...
                // Warlock
                warlockSetTarget();
                // Check for deputy promotion on TimespaceManager disconnect
    ...
    public static void Prefix(PlayerPhysics __instance) {
        bool correctOffset = Camouflager.camouflageTimer <= 0f && !TheDoraemon.Helpers.MushroomSabotageActive() && ((Morphling.morphling != null && __instance.myPlayer == Morphling.morphling && Morphling.morphTimer > 0f)
             || (MimicA.mimicA != null && __instance.myPlayer == MimicA.mimicA && MimicA.isMorph && MimicK.mimicK != null));
        correctOffset = correctOffset && !(Morphling.morphTimer > 0f) && !MimicA.isMorph;
        }
    }
    ...
    class PlayerControlCmdReportDeadBodyPatch {
        public static bool Prefix(PlayerControl __instance) {
            ...
            TheDoraemon.Helpers.handleVampireBiteOnBodyReport();
            TheDoraemon.Helpers.HandleUndertakerDropOnBodyReport();
            TheDoraemon.Helpers.handleTrapperTrapOnBodyReport();
            return true;
        }
    }
                ...
                if (deadPlayer != null && deadPlayer.killerIfExisting != null) {
                    ...
                    if (isMedicReport) {
                        ...
                        if (timeSinceDeath < Detective.reportNameDuration * 1000) {
                            ...
                            var typeOfColor = TheDoraemon.Helpers.isLighterColor(deadPlayer.killerIfExisting.Data.DefaultOutfit.ColorId) ? ModTranslation.getString("detectiveColorLight") : ModTranslation.getString("detectiveColorDark");
    ...
    public static class AssignRolePatch
    {
        ...
        {
            if ((player == SchrodingersCat.schrodingersCat && !SchrodingersCat.hasTeam() && !SchrodingersCat.isExiled) || FreePlayGM.isFreePlayGM)
                return false;
            return true;
        }
    }
    ...
    public static class MurderPlayerPatch
    {
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)]PlayerControl target)
        {
            ...
            if ((target.hasFakeTasks()|| target == Pursuer.pursuer || target == Thief.thief || (target == Shifter.shifter && Shifter.isNeutral) || Madmate.madmate.Any(x => x.PlayerId == target.PlayerId) || target == CreatedMadmate.createdMadmate || target == JekyllAndHyde.jekyllAndHyde || target == Fox.fox) && !FreePlayGM.isFreePlayGM)
            ...
            if (NekoKabocha.nekoKabocha != null && target == NekoKabocha.nekoKabocha && __instance != NekoKabocha.nekoKabocha)
            {
                if (!__instance.Data.IsDead)
                {
                    if ((__instance.Data.Role.IsImpostor && NekoKabocha.revengeImpostor)
                        || (TheDoraemon.Helpers.isNeutral(__instance) && NekoKabocha.revengeNeutral)
                        || (NekoKabocha.revengeCrew && !TheDoraemon.Helpers.isNeutral(__instance) && !__instance.Data.Role.IsImpostor))
                    {
            ...
            TheDoraemon.Helpers.HandleRoleFlashOnDeath(target);
            ...
            if (Bait.bait != null && target == Bait.bait)
            {
                ...
                if (Bait.showKillFlash && __instance != Bait.bait && __instance == PlayerControl.LocalPlayer) {
                    TheDoraemon.Helpers.showFlash(new Color(204f / 255f, 102f / 255f, 0f / 255f));
                }
            }
            ...
            if (TimespaceManager.timespaceManager != null && PlayerControl.LocalPlayer == TimespaceManager.timespaceManager && __instance == TimespaceManager.timespaceManager)
            {
                TimespaceManager.acTokenChallenge.Value.isTriggeredFalse = false;

                if (TimespaceManager.acTokenChallenge.Value.cleared)
                {
                    foreach (var dp in deadPlayers)
                    {
                        if (dp.player == null || !TheDoraemon.Helpers.isEvil(dp.player)) continue;
                        if (dp.killerIfExisting != TimespaceManager.timespaceManager)
                        {
                            TimespaceManager.acTokenChallenge.Value.cleared = false;
                            break;
                        }
                    }
                    ...
                    foreach (PlayerControl p in PlayerControl.AllPlayerControls)
                    {
                        if (p == null || !TheDoraemon.Helpers.isEvil(p)) continue;
                        if (!p.Data.IsDead)
                        {
                            TimespaceManager.acTokenChallenge.Value.isTriggeredFalse = true;
                            break;
                        }
                    }
                }
            ...
            if (Teleporter.teleporter != null && PlayerControl.LocalPlayer == Teleporter.teleporter)
            {
                ...
                if (teleportT != byte.MaxValue) teleportTarget = TheDoraemon.Helpers.playerById(teleportT);
                if (teleportTarget != null)
                {
                    if (target.PlayerId != teleportT && DateTime.UtcNow.Subtract(Teleporter.acTokenChallenge.Value.swapTime).TotalSeconds <= 4f)
                    {
                        if (Teleporter.acTokenChallenge.Value.target2 != byte.MaxValue && (teleportTarget == Snitch.snitch || teleportTarget == Mayor.mayor || teleportTarget == Guesser.niceGuesser || teleportTarget == FortuneTeller.fortuneTeller
                            || teleportTarget == TimespaceManager.timespaceManager))
            ...
            {
                TheDoraemon.Helpers.showFlash(new Color(42f / 255f, 187f / 255f, 245f / 255f), message: ModTranslation.getString("evilTrackerInfo"));
            }
        ...
        if (__instance.Data.Role.IsImpostor)
        {
            if (target) _ = new StaticAchievementToken("spy.another1");
            else
            {
                if (!TheDoraemon.Helpers.AnyNonTriggersBetween(PlayerControl.LocalPlayer.GetTruePosition(), target.GetTruePosition(), out var vec)
        ...
        if (Undertaker.undertaker != null && PlayerControl.LocalPlayer == Undertaker.undertaker && target == Undertaker.undertaker && Undertaker.DraggedBody != null)
            ...
            if (Blackmailer.blackmailer != null && PlayerControl.LocalPlayer == Blackmailer.blackmailer && __instance == Blackmailer.blackmailer)
            {
                foreach (PlayerControl p in PlayerControl.AllPlayerControls)
                {
                    ...
                    if (!TheDoraemon.Helpers.AnyNonTriggersBetween(target.GetTruePosition(), p.GetTruePosition(), out var vec)
            ...
            if (MimicK.mimicK != null && __instance == MimicK.mimicK && target != MimicK.mimicK)
            {
                ...
                if (Camouflager.camouflageTimer <= 0f && !TheDoraemon.Helpers.MushroomSabotageActive())
                    MimicK.mimicK.setLook(target.Data.PlayerName, target.Data.DefaultOutfit.ColorId, target.Data.DefaultOutfit.HatId, target.Data.DefaultOutfit.VisorId, target.Data.DefaultOutfit.SkinId, target.Data.DefaultOutfit.PetId);
            ...
            {
                TheDoraemon.Helpers.showFlash(new Color(42f / 255f, 187f / 255f, 245f / 255f), message: ModTranslation.getString("mimicAInfo"));
            }
    ...
    public static class ExilePlayerPatch
    {
        public static void Postfix(PlayerControl __instance)
        {
            ...
            // Remove fake tasks when player dies
            if ((__instance.hasFakeTasks()|| __instance == Pursuer.pursuer || __instance == Thief.thief || (__instance == Shifter.shifter && Shifter.isNeutral) || Madmate.madmate.Any(x => x.PlayerId == __instance.PlayerId) || __instance == CreatedMadmate.createdMadmate || __instance == JekyllAndHyde.jekyllAndHyde || __instance == Fox.fox) && !FreePlayGM.isFreePlayGM)
                __instance.clearAllTasks();
