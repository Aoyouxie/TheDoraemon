    ...
    public static class Patches
    {
            ...
            public static void Postfix(GameSettingMenu __instance, int tabNum, bool previewOnly)
            {
                ...
                if (tabNum > 2)
                {
                    ...
                    if (tabNum >= 5)
                    {
                        ...
                        if (tabNum == 6)
                        {
                            SettingsUpdate.SpawnExternalButton(__instance, tab, ref num, "保存到新文件", () => SettingsUpdate.ExportSlot(__instance));
                        ...
                        SettingsUpdate.SpawnExternalButton(__instance, tab, ref num, "返回", () => Coroutines.Start(TabPatches.ChangeTab(__instance, 3)));
                    }
                    if (tabNum > 4) return;
        ...
        private class SettingsUpdate
        {
            ...
            public static void Postfix(GameSettingMenu __instance)
            {
                ...
                CreateSettings(__instance, 3, "ModSettings", "模组基础设置", settingsButton, MultiMenu.Main);
                CreateSettings(__instance, 4, "CrewSettings", "船员阵营设置", settingsButton, MultiMenu.Crewmate);
                CreateSettings(__instance, 5, "NeutralSettings", "独立阵营设置", settingsButton, MultiMenu.Neutral);
                CreateSettings(__instance, 6, "ImpSettings", "伪装者阵营设置", settingsButton, MultiMenu.Impostor);
                CreateSettings(__instance, 7, "ModifierSettings", "附加职业设置", settingsButton, MultiMenu.Modifiers);
                CreateSettings(__instance, 8, "ImportSettings", "保存预设", settingsButton, MultiMenu.External);
                CreateSettings(__instance, 9, "ExportSettings", "导出预设", settingsButton, MultiMenu.External);
            }
            ...
            public static void CreateSettings(GameSettingMenu __instance, int target, string name, string text, GameObject settingsButton, MultiMenu menu)
            {
                ...
                if (target < 8)
                {
                    ...
                    if (target == 3)
                    {
                        ImportText = SpawnExternalButton(__instance, tabOptions, ref num, "加载自定义预设", () => Coroutines.Start(TabPatches.ChangeTab(__instance, 8)));
                        ExportText = SpawnExternalButton(__instance, tabOptions, ref num, "导出自定义预设", () => Coroutines.Start(TabPatches.ChangeTab(__instance, 9)));
                    }
            ...
            public static void ExportSlot(GameSettingMenu __instance)
            {
                ...
                var text = Path.Combine(Application.persistentDataPath, "预设 1.txt");
                var i = 1;

                while (File.Exists(text))
                {
                    i++;
                    text = Path.Combine(Application.persistentDataPath, $"预设 {i}.txt");
                }
        ...
        class SettingsAwake
        {
            ...
            public static void Postfix(LobbyViewSettingsPane __instance)
            {
                ...
                CreateButton(__instance, 1, "ModTab", "基础设置", MultiMenu.Main, overview);
                CreateButton(__instance, 2, "CrewmateTab", "船员设置", MultiMenu.Crewmate, overview);
                CreateButton(__instance, 3, "NeutralTab", "独立设置", MultiMenu.Neutral, overview);
                CreateButton(__instance, 4, "ImpostorTab", "伪装者设置", MultiMenu.Impostor, overview);
                CreateButton(__instance, 5, "ModifierTab", "附加职业", MultiMenu.Modifiers, overview);
            }
