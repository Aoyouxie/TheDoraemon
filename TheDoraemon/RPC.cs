...
using static TheDoraemon.HudManagerStartPatch;
using static TheDoraemon.GameHistory;
using static TheDoraemon.TORMapOptions;
...
namespace TheDoraemon
{
    ...
    public static void HandleShareOptions(byte numberOfOptions, MessageReader reader) {            
            try {
                ...
                TheDoraemonPlugin.Logger.LogError("Error while deserializing options: " + e.Message);
            }
        }
        ...
        public static void workaroundSetRoles(byte numberOfRoles, MessageReader reader)
        {
                ...
                {                   
                    ...
                    try {
                        ...
                        TheDoraemonPlugin.Logger.LogError("Error while deserializing roles: " + e.Message);
                    }
            }
            ...
            foreach (PlayerControl player in PlayerControl.AllPlayerControls)
                if (player.PlayerId == playerId) {
                    ...
                    case RoleId.Engineer:
                        ...
                        break;
                    case RoleId.TimespaceManager:
                        TimespaceManager.timespaceManager = player;
                        break;
                    case RoleId.Deputy:
                ...
                if (p == 1f)
                {
                    ...
                    foreach (var task in PlayerControl.LocalPlayer.myTasks) {
                        if (ShipStatus.Instance.CommonTasks.Any(x => x.TaskType == task.TaskType)) {
                            ...
                            TheDoraemonPlugin.Logger.LogMessage($"Added {task.TaskType.ToString()} for common task");
                        }
                    }
        ...
        public static void shifterShift(byte targetId)
        {
            ...
            if (TimespaceManager.timespaceManager != null && TimespaceManager.timespaceManager == player)
            {
                if (TimespaceManager.formerDeputy != null && TimespaceManager.formerDeputy == TimespaceManager.timespaceManager) TimespaceManager.formerDeputy = oldShifter;  // Shifter also shifts info on promoted deputy (to get handcuffs)
                TimespaceManager.timespaceManager = oldShifter;
                TimespaceManager.onAchievementActivate();
            }
            if (TimespaceManager.formerSheriff != null && TimespaceManager.formerSheriff == player)
            {
                TimespaceManager.formerSheriff = oldShifter;
                TimespaceManager.onAchievementActivate();
                isHusk = true;
            }
        ...
        public static void deputyPromotes()
        {
            if (Deputy.deputy != null) {  // Deputy should never be null here, but there appeared to be a race condition during testing, which was removed.
                TimespaceManager.replaceCurrentSheriff(Deputy.deputy);
                if (PlayerControl.LocalPlayer == Deputy.deputy)
                {
                    _ = new StaticAchievementToken("deputy.another1");
                    TimespaceManager.onAchievementActivate();
                }
                TimespaceManager.formerDeputy = Deputy.deputy;
                Deputy.deputy = null;
                GameStatistics.recordRoleHistory(TimespaceManager.timespaceManager);
        ...
        public static void erasePlayerRoles(byte playerId, bool ignoreModifier = true, bool isCreatedMadmate = false, bool generateTasks = true) {
            ...
            if (player == TimespaceManager.timespaceManager) TimespaceManager.clearAndReload();
        ...
        {
            TheDoraemonPlugin.Logger.LogMessage($"blink fistjumpo: {isFirstJump}");
        ...
        public static void thiefStealsRole(byte playerId) {
            ...
            if (target == null) return;
            if (target == TimespaceManager.timespaceManager) {
                TimespaceManager.timespaceManager = thief;
                TimespaceManager.onAchievementActivate();
            }
