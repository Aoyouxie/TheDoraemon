    ...
    public static class Helpers
    {
        ...
        public static Texture2D loadTextureFromDisk(string path) {
            try {
                ...
                TheDoraemonPlugin.Logger.LogError("Error loading texture from disk: " + path);
            }
            return null;
        }

        /* This function has been removed from TDR because we switched to assetbundles for compressed audio. leaving it here for reference - Gendelo
        public static AudioClip loadAudioClipFromResources(string path, string clipName = "UNNAMED_TOR_AUDIO_CLIP") {
            ...
            // Usage example:
            //AudioClip exampleClip = Helpers.loadAudioClipFromResources("TheDoraemon.Resources.exampleClip.raw");
        ...
        public static string GradientColorText(string startColorHex, string endColorHex, string text)
        {
            ...
            {
                TheDoraemonPlugin.Logger.LogError("GradientColorText : Invalid Color Hex Code, Hex code should be 6 characters long (without #) (e.g., FFFFFF).");
                return text;
            }
        ...
        public static void enableCursor(bool initalSetCursor)
        {
            if (initalSetCursor)
            {
                Sprite sprite = loadSpriteFromResources("TheDoraemon.Resources.Cursor.png", 115f);
                Cursor.SetCursor(sprite.texture, Vector2.zero, CursorMode.Auto);
                return;
            }
            if (TheDoraemonPlugin.ToggleCursor.Value)
            {
        ...
        public static void generateNormalTasks(this PlayerControl player)
        {
            ...
            foreach (var task in ShipStatus.Instance.CommonTasks) {
                if (ShipStatusPatch.commonTasks.Any(x => x == task.TaskType)) {
                    ...
                    TheDoraemonPlugin.Logger.LogMessage($"Generated {task.TaskType.ToString()} for player {player.Data.PlayerName}");
                }
            }
        ...
        {
            renderer.sprite = loadSpriteFromResources("TheDoraemon.Resources.StatisticsBackground.png", 100f);
        ...
        public static bool hidePlayerName(PlayerControl source, PlayerControl target) {
            ...
            else if (Deputy.knowsSheriff && (source == TimespaceManager.timespaceManager || source == Deputy.deputy) && (target == TimespaceManager.timespaceManager || target == Deputy.deputy)) return false; // TimespaceManager & Deputy see the names of each other
        ...
        public static MurderAttemptResult checkMuderAttempt(PlayerControl killer, PlayerControl target, bool blockRewind = false, bool ignoreBlank = false, bool ignoreIfKillerIsDead = false) {
            ...
            else if (checkArmored(target, true, killer == PlayerControl.LocalPlayer, TimespaceManager.timespaceManager == null || killer.PlayerId != TimespaceManager.timespaceManager.PlayerId || (isEvil(target) && TimespaceManager.canKillNeutrals) || isKiller(target)))
            {
                return MurderAttemptResult.BlankKill;
            }
        ...
        public static void shareGameVersion() {
            ...
            writer.Write((byte)TheDoraemonPlugin.Version.Major);
            writer.Write((byte)TheDoraemonPlugin.Version.Minor);
            writer.Write((byte)TheDoraemonPlugin.Version.Build);
            writer.Write(AmongUsClient.Instance.AmHost ? Patches.GameStartManagerPatch.timer : -1f);
            writer.WritePacked(AmongUsClient.Instance.ClientId);
            writer.Write((byte)(TheDoraemonPlugin.Version.Revision < 0 ? 0xFF : TheDoraemonPlugin.Version.Revision));
            writer.Write(Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId.ToByteArray());
            AmongUsClient.Instance.FinishRpcImmediately(writer);
            RPCProcedure.versionHandshake(TheDoraemonPlugin.Version.Major, TheDoraemonPlugin.Version.Minor, TheDoraemonPlugin.Version.Build, TheDoraemonPlugin.Version.Revision, Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId, AmongUsClient.Instance.ClientId);
        }
        ...
        public static void toggleZoom(bool reset=false) {
            ...
            if (tzGO != null)
            {
                ...
                rend.sprite = zoomOutStatus ? loadSpriteFromResources("TheDoraemon.Resources.Plus_Button.png", 100f) : loadSpriteFromResources("TheDoraemon.Resources.Minus_Button.png", 100f);
                rendActive.sprite = zoomOutStatus ? loadSpriteFromResources("TheDoraemon.Resources.Plus_ButtonActive.png", 100f) : loadSpriteFromResources("TheDoraemon.Resources.Minus_ButtonActive.png", 100f);
        ...
        public static async Task checkBeta() {
            if (TheDoraemonPlugin.betaDays > 0) {
                TheDoraemonPlugin.Logger.LogMessage($"Beta check");
                ...
                TheDoraemonPlugin.Logger.LogMessage($"Compiled at {compileTime.ToString(CultureInfo.InvariantCulture)}");
                DateTime? now;
                ...
                try {
                    ...
                    if (response.IsSuccessStatusCode)
                        ...
                        TheDoraemonPlugin.Logger.LogMessage($"Could not get time from server: {response.StatusCode}");
                        now = DateTime.UtcNow; //In case something goes wrong. 
                    }
                } catch (System.Net.Http.HttpRequestException) {
                    now = DateTime.UtcNow;
                }
                if ((now - compileTime)?.TotalDays > TheDoraemonPlugin.betaDays) {
                    TheDoraemonPlugin.Logger.LogMessage($"Beta expired!");
                ...
                } else TheDoraemonPlugin.Logger.LogMessage($"Beta will remain runnable for {TheDoraemonPlugin.betaDays - (now - compileTime)?.TotalDays} days!");
            }
        }
