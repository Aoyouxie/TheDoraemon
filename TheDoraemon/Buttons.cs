...
namespace TheDoraemon
{
    ...
    static class HudManagerStartPatch
    {
        ...
        public static void setCustomButtonCooldowns() {
            ...
            sheriffKillButton.MaxTimer = TimespaceManager.cooldown;
            ...
            eraserButton.MaxTimer = Jackal.cooldown;
        ...
        /*public static void createRoleSummaryButton(HudManager __instance)
        {
            ...
            () => { },
            Helpers.loadSpriteFromResources("TheDoraemon.Resources.HelpButton.png", 150f),
        ...
        public static void createButtonsPostfix(HudManager __instance) {
            ...
            sheriffKillButton = new CustomButton(
                () => {
                    ...
                    if (murderAttemptResult is MurderAttemptResult.PerformKill or MurderAttemptResult.ReverseKill) {
                        byte targetId = 0;
                        if (((TimespaceManager.currentTarget.Data.Role.IsImpostor) ||
                            ...
                            (CreatedMadmate.createdMadmate == TimespaceManager.currentTarget && CreatedMadmate.canDieToSheriff) ||
                            (Madmate.canDieToSheriff && Madmate.madmate.Any(x => x.PlayerId == TimespaceManager.currentTarget.PlayerId))) &&
            ...
            deputyHandcuffButton = new CustomButton(
                () => {
                    byte targetId = 0;
                    targetId = TimespaceManager.timespaceManager == PlayerControl.LocalPlayer ? TimespaceManager.currentTarget.PlayerId : Deputy.currentTarget.PlayerId;  // If the deputy is now the timespaceManager, sheriffs target, else deputies target
                ...
                () => { return ((Deputy.deputy != null && Deputy.deputy == PlayerControl.LocalPlayer) || (TimespaceManager.timespaceManager != null && TimespaceManager.timespaceManager == PlayerControl.LocalPlayer && TimespaceManager.timespaceManager == TimespaceManager.formerDeputy && Deputy.keepsHandcuffsOnPromotion)) && !PlayerControl.LocalPlayer.Data.IsDead; },
                () => {
                    if (deputyButtonHandcuffsText != null) deputyButtonHandcuffsText.text = $"{Deputy.remainingHandcuffs}";
                    return ((Deputy.deputy != null && Deputy.deputy == PlayerControl.LocalPlayer && Deputy.currentTarget) || (TimespaceManager.timespaceManager != null && TimespaceManager.timespaceManager == PlayerControl.LocalPlayer && TimespaceManager.timespaceManager == TimespaceManager.formerDeputy && TimespaceManager.currentTarget)) && Deputy.remainingHandcuffs > 0 && PlayerControl.LocalPlayer.CanMove;
                },
                () => { deputyHandcuffButton.Timer = deputyHandcuffButton.MaxTimer; },
                ...
                () => {
                    if (!Helpers.isMira()) {
                        if (SecurityGuard.minigame == null) {
                            ...
                            UnityEngine.Object.FindObjectsOfType<SystemConsole>().ToList().ForEach(x => TheDoraemonPlugin.Logger.LogMessage($"{x.name} {x.GetType()}, {x.MinigamePrefab.TaskType}"));
            ...
            accelAttributeButton = new CustomButton(
                ...
                () => { },
                Helpers.loadSpriteFromResources("TheDoraemon.Resources.AccelAttribute.png", 250f),
            ...
            decelAttributeButton = new CustomButton(
                ...
                () => { },
                Helpers.loadSpriteFromResources("TheDoraemon.Resources.DecelAttribute.png", 250f),
            ...
            yoyoButton = new CustomButton(
                () => {
                    ...
                    {
                        TheDoraemonPlugin.Logger.LogMessage($"marked location is null in button press");
                    ...
                    {
                        TheDoraemonPlugin.Logger.LogMessage("in else for some reason");
                        // Jump to location
                        TheDoraemonPlugin.Logger.LogMessage($"trying to blink!");
